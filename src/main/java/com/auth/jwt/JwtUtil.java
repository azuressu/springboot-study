...
import com.auth.entity.UserRoleEnum;
...
public class JwtUtil { // Util: 다른 객체에 의존하지 않고 하나의 모듈로서 동작하는 클래스
    ...
    public String createToken(String username, UserRoleEnum role) {
        ...
        return BEARER_PREFIX +
                Jwts.builder()
                        .setSubject(username)           // 사용자 식별자값(ID도 가능)
                        .claim(AUTHORIZATION_KEY, role) // 사용자 권한 (Key, Value)
                        .setExpiration(new Date(date.getTime() + TOKEN_TIME)) // 만료 시간(현재 시간+1시간)
                        .setIssuedAt(date) // 발급일(현재 날짜)
    ...
    // Cookie에 들어있는 JWT를 SubString (Bearer 을 제거)
